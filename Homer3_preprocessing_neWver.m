...
for i = 1:xls_num
    ...
    intensity = xlsread ([file_path xls_name]);
    SS = intensity (:, 1:44);
    LS = intensity (:, 45:88);
    d_nColumns = size (intensity, 1);
    temp_d = [SS; LS];
    data = reshape (temp_d(:), d_nColumns, []); % 調整正確.nirs欄位順序 (扣完一起調整),相同波長緊鄰並排,ex: C1W1 C2W1 C1W2 C2W2
